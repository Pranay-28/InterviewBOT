...
function StartInterview({ params }) {
  ...
  const GetInterviewDetails = async () => {
    ...
    console.log(result);
    try {
      let jsonMockResp = JSON.parse(result[0].jsonMockResp.trim());
      // Ensure we always have an array (Groq may have wrapped it in an object)
      if (!Array.isArray(jsonMockResp)) {
        jsonMockResp = Object.values(jsonMockResp).find(v => Array.isArray(v)) ?? [];
      }
